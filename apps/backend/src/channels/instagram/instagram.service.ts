...
export class InstagramService {
  ...
  async handleInbound(rawBody: Buffer, signature: string): Promise<void> {
    ...
    try {
      ...
      this.logger.warn('Instagram: failed to parse JSON body');
      return;
    }
    ...
    if (!messageText) {
      this.logger.warn(`Instagram: non-text message from ${senderId}, skipping`);
      return;
    }
    ...
    if (result.status === 'duplicate') {
      this.logger.debug(`Instagram: duplicate message ${mid}, skipping AI`);
      return;
    }
  ...
  async send(recipientId: string, content: string): Promise<void> {
    const INSTAGRAM_ACCESS_TOKEN = this.configService.get<string>('INSTAGRAM_ACCESS_TOKEN');
